...
function Audit-CISMEx122
{
	try
	{
		...
		if ($Count -igt 0)
		{
			$MBX | ForEach { Get-MgUser -UserId $_.ExternalDirectoryObjectId } | Format-Table -AutoSize DisplayName, UserPrincipalName, AccountEnabled | Out-File "$path\CISMEx122SharedMailboxesSignIn.txt"
