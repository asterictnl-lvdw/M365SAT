...
{	
	Try
	{
		...
		Search-UnifiedAuditLog -StartDate $startDate -EndDate $endDate | Where-Object { $_.Operations -eq "add user." } | Format-Table -AutoSize | Out-File "$path\CISMEx231AccountProvisioningActivityReport.txt"
		$endobject = Build-CISMEx231("0")
		Return $endobject
	}
	catch
	{
