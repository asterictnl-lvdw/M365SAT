...
function Audit-CISMOff114
{
	Try
	{
		$GuestUsers = (Get-MgUser -All -Property UserType, UserPrincipalName | Where { $_.UserType -ne "Member" })
		
		If ($GuestUsers.Count -igt 0)
		{
			$GuestUsers | Format-Table -AutoSize UserPrincipalName, UserType | Out-File "$path\CISMOff114GuestAccounts.txt"
