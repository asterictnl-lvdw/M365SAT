...
function Inspect-MailboxesWithIMAPEnabled
{
	...
	{
		$Buffer = @()
		$MailboxPlans = (Get-CASMailboxPlan -Filter { ImapEnabled -eq "true" })
		foreach ($Plan in $MailboxPlans)
		{
			if ($Plan.IMAPEnabled -eq $True)
			{
				$Plan.Id += $Buffer
			}
		}
		if ($Buffer.Count -igt 0)
		{
			$endobject = Build-MailboxesWithIMAPEnabled($Buffer.Count)
			Return $endobject
		}
	}
	Catch
