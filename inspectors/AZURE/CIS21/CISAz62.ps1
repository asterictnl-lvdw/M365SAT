...
# Purpose: Ensure that SSH access from the Internet is evaluated and restricted
...
function Build-CISAz62($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz62"
		FindingName	     = "CIS Az 6.2 - SSH access from the Internet is evaluated and restricted"
...
function Audit-CISAz62
{
	try
	{
		...
		{
			$finalobject = Build-CISAz62($Violation)
			return $finalobject
		}
		return $null
	}
