...
function Build-CISAz5210($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz5210"
		FindingName	     = "CIS Az 5.2.10 - Activity Log Alert does not exist for Delete Public IP Address rules"
...
function Audit-CISAz5210
{
	try
	{
		...
		if ($Violation.count -igt 0){
			$finalobject = Build-CISAz5210($violation)
			return $finalobject
		}
		return $null
	}
