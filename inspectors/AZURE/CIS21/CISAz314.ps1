...
function Build-CISAz314($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz314"
		FindingName	     = "CIS Az 3.14 - Some Storage Logging is disabled for Table Service for Read, Write and Delete Requests"
		...
		PowerShellScript = 'Set-AzStorageServiceLoggingProperty -ServiceType Table -LoggingOperations read,write,delete -RetentionDays 90 -Context $MyContextObject'
		DefaultValue	 = "None"
		ExpectedValue    = "All"
...
function Audit-CISAz314
{
	try
	{
		...
		if ($violation.Count -igt 0){
			$finalobject = Build-CISAz314($violation)
			return $finalobject
		}
		return $null
	}
