...
function Build-CISAz1230($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz1230"
		FindingName	     = "CIS Az 1.23 - No Custom Role is Assigned Permissions for Administering Resource Locks"
...
function Audit-CISAz1230
{
	try
	{
		...
		if ($ResourceLockAdministrators.Count -igt 0)
		{
			...
			$finalobject = Build-CISAz1230($ResourceLockAdministratorsList)
			return $finalobject
		}
		return $null
	}
