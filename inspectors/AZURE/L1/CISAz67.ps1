...
function Audit-CISAz67
{
	try
	{
		...
		{
			$endobject = Build-CISAz67 -ReturnedValue ($MethodsRequired.lockoutDurationInSeconds) -Status "FAIL" -RiskScore "3" -RiskRating "Low"
			return $endobject
		}
		else
		{
			$endobject = Build-CISAz67 -ReturnedValue ($MethodsRequired.lockoutDurationInSeconds) -Status "PASS" -RiskScore "0" -RiskRating "None"
			Return $endobject
		}
		return $null
	}
	...
	{
		$endobject = Build-CISAz67 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
