...
function Build-CISAz417($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz417"
		FindingName	     = "CIS Az 4.17 - Some Azure Storage Accounts have their 'Allow Blob Anonymous Access' set to Enabled"
...
function Audit-CISAz417
{
	try
	{
		...
		if ($violation.Count -igt 0){
			$finalobject = Build-CISAz417($violation)
			return $finalobject
		}
		return $null
	}
