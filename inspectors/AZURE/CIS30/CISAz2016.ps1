...
function Build-CISAz2016($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz2016"
		FindingName	     = "CIS Az 2.16 - Guest invite restrictions is not set to 'Only users assigned to specific admin roles can invite guest users'"
		...
		References	     = @(@{ 'Name' = 'Configure external collaboration settings'; 'URL' = 'https://learn.microsoft.com/en-us/entra/external-id/external-collaboration-settings-configure' },
...
function Audit-CISAz2016
{
	try
	{
		...
		{
			$finalobject = Build-CISAz2016($Policy.AllowInvitesFrom)
			return $finalobject
		}
		return $null
	}
