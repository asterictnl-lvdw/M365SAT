...
function Build-CISAz3174($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz3174"
		FindingName	     = "CIS Az 3.1.7.4 - Microsoft Defender for SQL Servers on Machines Is Set to 'Off'"
		ProductFamily    = "Microsoft Azure"
		RiskScore	     = "2"
		Description	     = "Enabling Microsoft Defender for SQL Servers on Machines allows for greater defense-in-depth, includes functionality for discovering and classifying sensitive data, surfacing and mitigating potential database vulnerabilities, and detecting anomalous activities that could indicate a threat to your database."
		Remediation	     = "Use the PowerShell script to remediate the issue"
		PowerShellScript = 'Set-AzSecurityPricing -Name "SqlServerVirtualMachines" -PricingTier "Standard"'
		DefaultValue	 = "Off"
		ExpectedValue    = "On"
...
function Audit-CISAz3174
{
	try
	{
		...
		{
			$finalobject = Build-CISAz3174($AzSecuritySetting.PricingTier)
			return $finalobject
		}
		return $null
	}
