...
function Build-CISAz613($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz613"
		FindingName	     = "CIS Az 6.1.3 - Some storage accounts containing containers with activity logs are not encrypted with Customer Managed Keys (CMK)"
		...
		RiskRating	     = "Low"
		Priority		 = "Low"
		References	     = @(@{ 'Name' = 'DP-5: Use customer-managed key option in data at rest encryption when required'; 'URL' = 'https://learn.microsoft.com/en-us/security/benchmark/azure/mcsb-data-protection#dp-5-use-customer-managed-key-option-in-data-at-rest-encryption-when-required' },
...
function Audit-CISAz613
{
	try
	{
		...
		if ($Violation.count -igt 0){
			$finalobject = Build-CISAz613($violation)
			return $finalobject
		}
		return $null
	}
