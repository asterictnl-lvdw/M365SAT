...
# Purpose: Ensure that Activity Log Alert exists for Delete Network Security Group
...
function Build-CISAz624($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz624"
		FindingName	     = "CIS Az 6.2.4 - Activity Log Alert does not exist for Delete Network Security Groups"
...
function Audit-CISAz624
{
	try
	{
		...
		if ($Violation.count -igt 0){
			$finalobject = Build-CISAz624($violation)
			return $finalobject
		}
		return $null
	}
