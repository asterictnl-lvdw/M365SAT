...
function Build-CISAz625($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz625"
		FindingName	     = "CIS Az 6.2.5 - Activity Log Alert does not exist for Create or Update Security Solutions"
...
function Audit-CISAz625
{
	try
	{
		...
		if ($Violation.count -igt 0){
			$finalobject = Build-CISAz625($violation)
			return $finalobject
		}
		return $null
	}
