...
# Purpose: Ensure that the Expiration Date is set for all Keys in Non-RBAC Key Vaults
...
function Build-CISAz332($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz332"
		FindingName	     = "CIS Az 3.3.2 - Expiration Date is not set for all Keys in Non-RBAC Key Vaults"
...
function Audit-CISAz332
{
	try
	{
		...
		{
			$finalobject = Build-CISAz332($Violation)
			return $finalobject
		}
		return $null
	}
