...
function Build-CISAz23($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz23"
		FindingName	     = "CIS Az 2.3 - Restrict non-admin users from creating tenants is set to No"
...
function Audit-CISAz23
{
	try
	{
		...
		{
			$finalobject = Build-CISAz23($AuthorizationPolicy.defaultUserRolePermissions.allowedToCreateTenants)
			return $finalobject
		}
		return $null
	}
