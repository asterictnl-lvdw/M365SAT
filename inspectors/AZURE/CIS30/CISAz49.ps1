...
function Build-CISAz49($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz49"
		FindingName	     = "CIS Az 4.9 - Private Endpoints are not used to access some Storage Accounts"
...
function Audit-CISAz49
{
	try
	{
		...
		if ($violation.Count -igt 0){
			$finalobject = Build-CISAz49($violation)
			return $finalobject
		}
		return $null
	}
