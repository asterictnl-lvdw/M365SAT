...
function Build-CISAz627($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz627"
		FindingName	     = "CIS Az 6.2.7 - Activity Log Alert does not exist for Create and Update SQL Server Firewall Rules"
...
function Audit-CISAz627
{
	try
	{
		...
		if ($Violation.count -igt 0){
			$finalobject = Build-CISAz627($violation)
			return $finalobject
		}
		return $null
	}
