...
function Build-CISMEx217($findings)
{
	...
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMEx217"
		FindingName	     = "CIS MEx 2.1.7 - Anti-Phishing policy not has been created"
...
function Inspect-CISMEx217
{
	...
	Try
	{
		try
		{
			$AntiPhishPolicy = Get-AntiPhishPolicy | Format-Table name, enabled, PhishThresholdLevel, EnableMailboxIntelligenceProtection, EnableMailboxIntelligence, EnableSpoofIntelligence | Where-Object { $_.IsDefault -eq $true }
			if ($AntiPhishPolicy.count -eq 0)
			{
				$AntiPhishPolicy = Get-AntiPhishPolicy | Format-Table name, enabled, PhishThresholdLevel, EnableMailboxIntelligenceProtection, EnableMailboxIntelligence, EnableSpoofIntelligence
			}
