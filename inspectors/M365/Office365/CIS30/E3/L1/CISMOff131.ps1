...
function Build-CISMOff131($findings)
{
	...
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMOff131"
		FindingName	     = "CIS MOff 1.3.1 - Passwords are not set to never expire"
...
function Audit-CISMOff131
{
	try
	{
		...
		{
			$AffectedOptions | Format-Table -AutoSize | Out-File "$path\CISMOff131-PasswordValidityPeriodInDays.txt"
