...
### Version 3.1 alpha
- **Linux PowerShell 7 support**: Resolved issues with Linux and PowerShell 7, improving compatibility.
- **CSV Compatibility**: Added .CSV support for both AZURE and M365 inspectors by changing the lay-out [#37](https://github.com/Karmakstylez/M365SAT/issues/37)
- **New Connection Mechanism**: The new connection mechanism tries to connect 3 times ensuring that users can try again if connection fails
- **New logger script improvement**: Rewritten the PoSHLogger script as it is now a switch parameter and it defaults the warning messages. It supports both Linux and Windows
- **Improvement of inspector accuracy**: There have been several improvements making inspectors more accurate, resulting into a better auditting experience.
- **Connection Tester**: A connection tester has been added for troubleshooting purposes. This is to test the mechanism if the current order is working properly.
- **Microsoft Teams connector issues**: [#67](https://github.com/Karmakstylez/M365SAT/issues/67) Fixed issues where some users where reporting that Microsoft Teams was not connecting properly resulting into not being able to audit Microsoft Teams related CIS inspectors.
**Fixed issues with several inspectors**: [#63](https://github.com/Karmakstylez/M365SAT/issues/63) [#62](https://github.com/Karmakstylez/M365SAT/issues/62) have been fixed by changing some things within the inspectors such as typos and other mechanisms.
**Auto OperatingSystem Detection**: Implemented automatic OS detection to determine the OS and PowerShell version so you do not have to manually prompt the operating system anymore. This is now done automatically.
**Update mechanism improvement**: Improved the mechanism for updating modules as it depends on the PowerShell version you are running as it only updates the necessary modules instead of the modules in the previous list. 
**Updated multiple PowerShell scripts**: To improve readability and performance, multiple PowerShell modules have been rewritten 
**M365SATTester Update**: I have improved the readability of the M365SATTester where auditors can now easily change the settings within the script.

## Version 3.1 beta

### Known issues
- There are some bugs within the .CSV file resulting into having a blank line and some duplicate references. This is under investigation and will be fixed in the next release.
- If accidently closing the tab the script may hang and result into a deadlock. I am investigating this and consider implementing a time-out mechanism to force timeout if no connection is made in a period of time.
- The PowerShell commands are not copy-able in the current version. This is due to the new mechanism. I will look into splitting the PowerShell commands once again once all commands are updated.
- There is an issue with the latest MicrosoftExchangeOnline cmdlet running on PowerShell 7 on Windows 10/11. The latest version requires WAM to be used as it does not work properly with the latest PowerShell 7. A workaround will be that we are going to force the Devicelogin 
- Web Account Manager (WAM) is not working properly with some Windows 11/10 machines as it conflicts with some cmdlets resulting into failure of initiating connections. This impacts Az, Graph and Exchange. This has been an issue only with PowerShell 7 for now. PowerShell 5 is being investigated. There are several workarounds which will be posted in [#68](https://github.com/Karmakstylez/M365SAT/issues/68)
- There are issues with Inspector 5125 as the URL https://main.iam.ad.ext.azure.com/api/LoginTenantBrandings/0 does not always return the value and results into a 404 if this is the case in a fix the value will be UNKNOWN. You can verify the value manually by reviewing the link and check if the slider is on or off.